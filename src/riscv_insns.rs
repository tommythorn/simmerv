...
pub const INSTRUCTIONS: [(u32, u32, &str); 264] = [
    ...
    // Zicbom — cache block management operations (NOPs in emulation)
    (0xfff07fff, 0x0000200f, "cbo.inval"),
    (0xfff07fff, 0x0010200f, "cbo.clean"),
    (0xfff07fff, 0x0020200f, "cbo.flush"),
    // Zicboz — cache block zero
    (0xfff07fff, 0x0040200f, "cbo.zero"),
    // Zicbop — prefetch hints (NOPs in emulation)
...
mod test {
    ...
    impl RiscvDecoder for Tester<'static> {
        ...
        fn cbo_inval(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "cbo_inval";
            0
        }
        fn cbo_clean(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "cbo_clean";
            0
        }
        fn cbo_flush(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "cbo_flush";
            0
        }
        fn cbo_zero(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "cbo_zero";
            0
        }
        fn prefetch_i(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "prefetch_i";
            0
        }
        fn prefetch_r(_a: u64, _w: u32, s: &mut Self::Context) -> usize {
            s.s = "prefetch_r";
            0
        }
