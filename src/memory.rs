...
pub const MEMORY_BASE: u64 = 0x80000000;
...
impl Memory {
    ...
    pub fn read_u16(&mut self, p_address: u64) -> u16 {
        debug_assert!(
            p_address >= MEMORY_BASE && p_address.wrapping_add(1) >= MEMORY_BASE,
            "Memory address must equals to or bigger than DRAM_BASE. {p_address:X}"
        );
        let address = p_address - MEMORY_BASE;
    ...
    pub fn read_u32(&mut self, p_address: u64) -> u32 {
        debug_assert!(
            p_address >= MEMORY_BASE && p_address.wrapping_add(3) >= MEMORY_BASE,
            "Memory address must equals to or bigger than DRAM_BASE. {p_address:X}"
        );
        let address = p_address - MEMORY_BASE;
        let address = address as usize;
    ...
    pub fn read_u64(&mut self, p_address: u64) -> u64 {
        debug_assert!(
            p_address >= MEMORY_BASE && p_address.wrapping_add(7) >= MEMORY_BASE,
            "Memory address must equals to or bigger than DRAM_BASE. {p_address:X}"
        );
        let address = p_address - MEMORY_BASE;
        let address = address as usize;
    ...
    pub fn write_u8(&mut self, pa: u64, b: u8) -> Result<(), ()> {
        let offset = pa.wrapping_sub(MEMORY_BASE) as usize;
    ...
    // - operates on physical addresses (u64)
    // - uses only u64 values
    ...
    /// # Errors
    /// If any part of the access is outside memory, an unit error is returned
    pub fn slice(&mut self, pa: u64, size: usize) -> Result<&mut [u8], ()> {
        let pa = pa.wrapping_sub(MEMORY_BASE);
        if pa <= self.0.len() as u64 && pa + size as u64 <= self.0.len() as u64 {
            let pa = pa as usize;
