...
mod tests {
    use super::*;
    use crate::rvc::RVC64_EXPANDED;

    /// Helper to compare Uop fields (ignoring Op and insn_size since those
    /// differ by design)
    fn uops_equal_except_op(lhs: &Uop, rhs: &Uop) -> bool {
        lhs.rd == rhs.rd
            && lhs.rs1 == rhs.rs1
            && lhs.rs2 == rhs.rs2
            && lhs.rs3 == rhs.rs3
    ...
    /// Comprehensive test: iterate through all u16 values and verify that
    /// cpu::decode(a, x) produces the same Uop as cpu::decode(a,
    /// RVC64_EXPANDED[x])
    #[test]
    fn test_all() {
        use crate::cpu;
        ...
        for x in 0..=u16::MAX {
            // Skip odd 4-aligned values (per RISC-V spec)
            if x % 4 == 3 {
                continue;
            }
            ...
            let a = 0x80000000u64;
            let x_u32 = x as u32;
            let expanded = RVC64_EXPANDED[x as usize];

            // Decode the expanded 32-bit form
            let uop_ref = cpu::decode(a, expanded);

            // Decode the u16 directly
            let uop_new = cpu::decode(a, x_u32);

            // Compare (ignoring op)
            if !uops_equal_except_op(&uop_new, &uop_ref) {
                failures.push(format!(
                    "{x:04x}: {expanded:08x} ref={uop_ref:#?}, got new={uop_new:#?}"
                ));
            }
        }

        if !failures.is_empty() {
            eprintln!("Failures (showing first 20):");
            for failure in failures.iter().take(20) {
                eprintln!("  {}", failure);
            }
            panic!(
                "{} mismatches found out of ~65536 u16 values",
                failures.len()
            );
        }
    }
